...
(function(window, document, Math) {
	...
	var utils = (function() {
		...
		me.click = function(e) {
			...
			// jie-start
			var bool = target.tagName === "INPUT" && target.type === "file"
			if (!(/(SELECT|INPUT|TEXTAREA)/i).test(target.tagName) || bool) {
			// jie-end
		...
		// jie-start 增加长按功能
		me.customLongPress = function(e, eventName) {
			var ev = document.createEvent('Event');
			ev.initEvent('customLongPress', true, true);
			ev.pageX = e.pageX;
			ev.pageY = e.pageY;
			e.target.dispatchEvent(ev);
		};
	...
	function IScroll(el, options) {
		...
		// jie-start 是否启用'长按'事件
		this.options.customLongPress = false
		// jie-end 是否启用'长按'事件
		...
		// jie start
		this.__refreshFormInputlisten__();
		// jie end
	}
	...
	IScroll.prototype = {
		...
		_start: function(e) {
			...
			// jie-start
			// 当前活动元素是'input'或'textarea', 并且此元素在滚动条内时,直接返回,不发生滚动;
			var el = document.activeElement;
			// if (el.nodeName.toLowerCase() == 'input' || el.nodeName.toLowerCase() == 'textarea') {
				if (el._isInsideIscroll_) {
					return;
				}
			// }
			// jie-end
		...
		_move: function(e) {
			if (e.type !== 'mousemove' && this.options.movePreventDefault) {
				e.preventDefault();
			}
		...
		_end: function(e) {
			...
			// jie start 是否阻止在pc上的click事件
			if (!isMobile) {
				absDistX = Math.abs(this.distX);
				absDistY = Math.abs(this.distY);
				this.stopClick = absDistX > 10 || absDistY > 10 || duration > 300
			}
			// jie end
			...
			// jie-start 增加长按功能
			if (this.options.customLongPress && !this.moved && duration > 300) {
				utils.customLongPress(e);
				return;
			}
			// jie-end
		...
		// jie start  移动设备上聚焦时阻止滚动
		__refreshFormInputlisten__: function () {
			if (isMobile) {
				var that = this,
					formDoms = this.wrapper.querySelectorAll("input, textarea");

				// 先取消之前的绑字
				that._prev_formDoms_ && Array.prototype.forEach.call(that._prev_formDoms_, function(item) {
					item.removeEventListener('focusin', item.handler)
				})
				...
				// 如果是苹果设备，则在聚焦时禁止滚动条滚动, 在start里有判断
				Array.prototype.forEach.call(formDoms, function(item) {
					item.handler = focusinHandler
					item.addEventListener('focusin', item.handler)
				})
				this._prev_formDoms_ = formDoms
			}
		},
		// jie end

		// jie start IOS设备上聚焦时阻止滚动； 安卓设备不阻止滚动，但在input事件时，滚动到相应元素。
		/*__refreshFormInputlisten__: function () {
			var that = this,
				formDoms = this.wrapper.querySelectorAll("input, textarea");
			...
			if(isIDevice) {
				// 先取消之前的绑字
				that._prev_formDoms_ && Array.prototype.forEach.call(that._prev_formDoms_, function(item) {
					item.removeEventListener('focusin', item._iosd_)
				})
				...
				Array.prototype.forEach.call(formDoms, function(item) {
					item._iosd_ = iosd
					item.addEventListener('focusin', item._iosd_)
				})				
			} else {				
				// 先取消之前的绑字
				that._prev_formDoms_ && Array.prototype.forEach.call(that._prev_formDoms_ , function(item) {
					item.removeEventListener('input', item._ard_)
				})

				var ard = function (e) {
					console.log("android");
					that.scrollToElement(e.target, 0);
				}
				// 如果是安卓或其它设备，则在“输入”时让滚动条滚动到聚焦的dom元素;
				Array.prototype.forEach.call(formDoms, function(item) {
					item._ard_ = ard
					item.addEventListener('input', item._ard_)
				})
			}
			this._prev_formDoms_ = formDoms
		},*/
