...
Component({
    ...
    methods:{
        ...
        dispatcher(event){
            ...
            if (item.awawtype  && ((method=='attend') || (method=='cost') || (method=='costDynamic') || (method=="try") || (method=="absent"))){
                wx.showToast({title:`学员${item.name}的状态为${item.awawtype}，不能更改。`,icon:'none'});
                return;
            }
