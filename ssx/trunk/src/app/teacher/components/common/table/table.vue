            ...
            .heard-right {
                ...
                .item-wrapper {
                    ...
                    .item {
                        ...
                        position: relative;
                        .icon-wrapper {
                            @include position-absolute(-1px, 0, false, false);
                            font-size: 20px;
                        }
                        .heard-content {
                            width: 100%;
    ...
    <div class="wrapper">
        <div class="heard">
            <div class="heard-left" :style="{width: tableWidth+'px'}" @click="openSelectType('campus')">
                <div class="xiaoqu" ref="topLeft">{{name}}
                    <svg class="icon" aria-hidden="true" v-if="isClickTop">
            ...
            <div class="heard-right">
                <div class="item-wrapper" ref="topScroll">
                    <div class="item" v-for="(item,key,index) in customerType" :style="{width: tableWidth+'px'}">
                        <div class="icon-wrapper" @click="delItem(item,index)">
            ...
            @loadData="loadData">
               <div class="center-wrapper">
                   <div class="left-wrapper" :style="{width: tableWidth+'px'}">
                       <div class="body-left" v-for="card in list" @click="jumpRouter(card)">
                           <div class="xiaoqu">
                               {{card.name}}
                           </div>
                           <div class="icon-wrapper" v-if="isClickLeft">
                                <svg class="icon" aria-hidden="true">
                                   <use xlink:href="#icon-youjiantou"></use>
                                </svg>
                           </div>
        ...
        <div class="bottom">
            <div class="bottom-left" :style="{width:tableWidth+'px'}">
                <div class="xiaoqu" ref="bottomLeft">合计</div>
            </div>
    ...
    export default {
        ...
        methods: {
            loadData(ajaxPromise) {
                this.$emit('loadData', ajaxPromise)
            },
            scrollMove(pos) {
            ...
            delItem(obj, index) {
                ...
                this.scrollGoBack()
                this.$emit('changeRecordObj', this.recordObj)
            },
            openSelectType(str) {
                if (!this.isClickTop && str == 'campus') return
            ...
            jumpRouter(obj) {
                if (!this.isClickLeft) return 
                this.$router.push({name: 'cusAnalysisStatusDetail', params: {campusId: obj.CampusID}})
            }
        },
        mounted() {
            this.setFrameWidth()
        ...
        watch: {
            midList(val) {
                if (val) {
                    this.list = app.tool.clone(val)
                }
            },
            topAndBottomObj(val) {
                if (val) {
                    this.customerType = app.tool.clone(val)
                }
            },
            activeList(val) {
                if (val) {
                    this.typeList = app.tool.clone(val)
                }
            }
        },
        components: {
