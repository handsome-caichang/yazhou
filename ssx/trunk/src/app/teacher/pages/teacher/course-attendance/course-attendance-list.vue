...
<template>
	<div class="course-atds-list">
		...
		<scroller-base class="class-list-container" :data="list">
			<router-link class="class-item" v-for="(item,index) in list" tag="div" :key="item.id" :to="{name:'courseAttendanceStudents', query:{id: item.courseid}}">
				...
				<div class="class-item-room">
					<div>
						<div>上课教室：{{item.classroomname}}</div>
						<svg class="icon" aria-hidden="true">
				...
				<div class="item-left">
					...
					<div class="class-item-room">
						...
						<div class="class-room">{{item.classroomname}}</div>
						<!--上课时间-->
						<div class="class-date" v-if="item.duration != '-'">{{item.starttime | formatDatetime('hh:mm')}}~ {{item.endtime | formatDatetime('hh:mm')}}
						</div>
						<!--已上课||未上课标识-->
						<div>
		...
		<router-link to="/courseAttendanceNewCourse" tag="div" class="quick-sort-course">快速排课</router-link>
		<loading class="loading" v-show="isLoading" :bgType='bgType'></loading>
	</div>
</template>

<script>
import { getcourseattendancwx } from "teacher/api/course-attendance";
...
export default {
	...
	data() {
		return {
			...
			CFG: {
					dateChange: app.sysInfo.courseconfirmdatechange == "1" //是否可以更改日期
				}
			};
		},
		methods: {
			...
			getAjaxData() {
				...
				let param = {
					role: 1,
					starttime:  this.dateInfo.date,
					endtime : this.dateInfo.date,
					/*page:{
						pageindex:1,
						pagesize:20,
					}*/
				}
				return getcourseattendancwx(param).then(res => {
					this.isLoading = false;
					this.list = res.getcourseattendancwxs;
				})
			}
		},
		mounted() {
	...
	function getDateByDay(date, num) {
		...
		return {
			date: date.getFullYear() + '-' + fixZero(date.getMonth() + 1,2) + '-' + fixZero(date.getDate(),2),
