                ...
                <div class="body">
                    ...
                    <div class="header">
                        ...
                        <div class="info">
                            ...
                            <span class="info-user" @click="()=>{app.gotoIM({toUserID: detail.teacheruserid})}">{{detail.nickname}}
                    ...
                    <div class="content">
                        ...
                        <video-area v-if="detail.vediofiles.length>0"
                            class="content-video"
                            :edit="false"
                            :videoFileList="detail.vediofiles"
                            v-show="isShowMoreContent"></video-area>
                        <file-area v-if="detail.filefiles.length>0"
                            class="content-video"
                            :edit="false"
                            :fileList="detail.filefiles"
                            v-show="isShowMoreContent"></file-area>
                        <div class="link-wrap" v-if="detail.linkfiles&&detail.linkfiles.length" v-show="isShowMoreContent">
                            <div class="link" v-for="(item,index) in detail.linkfiles" :key="index">
...
<script>
    import {getjobinfobyid,replyjobbystudent} from 'parent/api/homework.js';
	...
	export default {
		...
		methods: {
			...
			getDetail(){ //获取详情
                let getWorkPromise = getjobinfobyid({
                        jobid: this.queryPara.id
                    }),
                ...
                Promise.all([getWorkPromise,getReplyPromise]).then(res => {
					...
					if (res[0].result.code == app.errok && res[1].errcode == app.errok) {
                        ...
                        this.errorFlag = true;
                        this.errorTips = res[0].result.msg;
                    }
                })
            },
            handlerComment(){
            ...
            save(type, medias, videos, attachments){
                let temp = this.voiceDelete.concat(this.imageDelete,this.videoDelete,this.attachmentsDelete),
                    saveObj = {
                        ...
                        //TODO: 改：
                        // content: app.tool.arrowFilter(this.content), //提交内容
                        // studentid: '', //学员id
                        // istemp: type, //提交类型：1是草稿，0是直接提交
                        // jobid: this.queryPara.id, //作业id
                        // imagesmediaids: [],//图片文件的微信media_id
                        // audiomediaids: [],
                        // vediofilelist: [],
                        // filefilelist: [],
                        // linkfilelist: []
                ...
                replyjobbystudent(saveObj).then(res => {
                    this.isLoading = false;
