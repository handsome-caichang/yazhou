...
axios.ajaxGet = function(url, args) {
    ...
    let qUrl = serverQuery.ip ? (url.startsWith('/')?`${serverQuery.ip}${url}`:`${serverQuery.ip}/${url}`) : url
...
axios.ajax = function (url, args) {
    ...
    return axios.post(
        qUrl, 
        qs.stringify({...args,UsePlatform:1})
    ).then(res => {
    	let errorcode = res.data.ErrorCode || res.data.errcode,
    		errormsg = res.data.ErrorMsg || res.data.errmsg;
        if (errorcode !== 200) { //服务端返回的状态码不是200时
...
axios.wxajax = function (name, args, type = 'post') {
    return axios.ajax(`http://10.0.0.30:8001/server/index.php?g=Web&c=Mock&o=mock&projectID=48&uri=${name}`, args, type)
}
