...
<template>
    <div class="buke-manage-container">
        ...
        <div class="body">
            <scroller-super
                class="scroller"
                :class="{'bottom-distance':pagingOption.params.status!==0}"
                :type="2"
                :data="list"
                ...
                ref="bukeScroller">
                <!--未安排补课-->
                <div class="card0 card" v-if="pagingOption.params.status==0" @click="selectStu(item)"
                ...
                <!--已安排补课未上课-->
                <div class="card1 card" v-if="pagingOption.params.status==1" v-for="(item,index) in list" :key="index">
                ...
                <!--已安排补课已上课-->
                <div class="card2 card" v-if="pagingOption.params.status==2"
            ...
            <empty-page class="noData-temp noList" :class="{'bottom-distance':pagingOption.params.status!=0}" type=1 text="还没有补课信息~" v-if="list.length == 0"></empty-page>
        </div>
        <div class="footer" v-show="pagingOption.params.status==0">
            <div><div class="btn btn-1" v-if="MissStudentDelete" @click="deleteStu">删除</div></div>
            <div>
...
<script>
    import {processPost, savePost, processGet,getdicinfobytypem,getclassroomlistmobile} from "teacher/api/common";
    import {getmendinfolistbymobile,getmendinfobymobile, getBukeClassroom,removemendbymobile} from 'teacher/api/buke';
            ...
            return {
                ...
                pagingOption: {
                    ...
                    params: {
                        ...
                        // teacherids: ''//老师id
                        campusid: '', //校区ID
                        query: '', //查询条件（学员姓名、学号、电话）
                        status: 0 //补课状态（0未补课 1已补课）
                    },
                    pageOpt:{
                        isdesc:false,
                        sortfiled:'',
        ...
        methods: {
                ...
                ajaxPromise.then(res => {
                    ...
                    if (res.result.code == app.errok) {
                        ...
                        res.data.forEach(item => {
                            ...
                            if(this.pagingOption.params.status == 0) {
                                item.checked = false;
                            }
                        });
            ...
            bukeAction(flag) {
                ...
                if (flag == 0) {
                    ...
                    //TODO:改！缺少接口！！获取教室信息 注意参数大小写
                    getclassroomlistmobile({
                        campusId: this.pagingOption.params.CampusId
                    }).then(res => {
                        console.log(res)
                        if (res.data.length) {
