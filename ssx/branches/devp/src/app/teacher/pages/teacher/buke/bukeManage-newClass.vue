    ...
    <div class="bukeManage-newClass-container">
        <scroller-base
            ...
            <div class="conflict" @click="selectCheckConflict" v-show="app.tool.op('IngoreCourseConflict')">
                ...
                <div class="val">
                    <svg class="icon" aria-hidden="true">
                        <use :xlink:href="savePara.checkconflict==1?'#icon-duoxuan':'#icon-duoxuan-weixuanze'"></use>
                    </svg>
                </div>
            </div>
            <div class="void"></div>
...
<script>
    ...
    import {savemend} from "teacher/api/buke.js";
    ...
    export default {
        ...
        methods: {
            ...
            //上课冲突
            selectCheckConflict() {
                this.savePara.checkconflict = this.savePara.checkconflict == 1 ? 0 : 1;
            },
            submit() {
                ...
                // 课程id
                this.savePara.shiftid = this.bukeManage.shift;
                // 校区id
                this.savePara.campusid = this.bukeManage.campusId;
                // 开始时间 结束时间
                this.savePara.startime = this.courseDate+' '+this.nowTime;
                this.savePara.endtime = this.courseDate+' '+this.endTime;
                //老师 助教
                let teacher = [{
                ...
                this.savePara.teachemployeeinfo = teacher.concat(assTeacher);
                //教室
                this.savePara.classroomid = this.selectedClassroomMsg.ID;
                //冲突  点击时已赋值
                //学生
                let ids = [];
                this.bukeManage.students.forEach(item => {
                    ids.push(item.ID);
                });
                this.savePara.studentids = ids;
                this.send();
            },
            send(){
                savemend(this.savePara).then(res=>{
                    if(res.result.code==app.errok){
                        app.toast('success', '安排补课成功。');
                        this.$router.back();
                        app.eventDefine.emit('BukeNewClass');
                    }else if(res.result.code==421){
