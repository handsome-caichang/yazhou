...
<template>
	<div class="commentRank-container">
		<div class="body">
			<scroller-super class="scroller" :type="2" :data="classList" :pagingOption="pagingOption" @loadData="loadData" ref="commentRankListIScroller">
				<!-- 搜索条件 -->
				<div class="condition">
					...
					<div class="date-sousuo">
						<date-bar class="date" :sdate='pagingOption.params.starttime' :edate='pagingOption.params.endtime' :index="quickDateIndex" @changeDate="changeDate">
						</date-bar>
				...
				<div class="class-table">
						...
						<span class="span">
							<span @click="filterScopeAndRank('Scope')">平均分
								<svg class="icon" aria-hidden="true">
									<use :xlink:href="pagingOption.params.sort === 'Scope' && pagingOption.params.desc === 1 ? '#icon-paixudidaogao' 
							...
							<span @click="filterScopeAndRank('MyRank')">排名
								<svg class="icon" aria-hidden="true">
									<use :xlink:href="pagingOption.params.sort === 'MyRank' && pagingOption.params.desc === 1 ? '#icon-paixudidaogao' 
					...
					<div class="comment-list">
						<div @click="toDimension(item.itemid)" v-for="item in list" :key="item.itemid" class="to-detail">
			...
			<state-actionsheet @stateType="acceptType" class="as-state" :stateList="stateFilter.arr" :curState="stateFilter.state" :opened.sync="stateFilter.opened">
			</state-actionsheet>
...
import {
    getteacherrank,
    getteachercoursecommentscope
} from "teacher/api/comment-rank";
...
export default {
    ...
    data() {
        return {
            ...
            pagingOption: {
                api: getteachercoursecommentscope,
                params: {
                    // sort: "Scope", // 默认平均分;MyRank,排名
                    // desc: 1,
                    // type: 5,
                    ...
                    // 默认显示7天
                    starttime:
                        app.tool.parseHash().query.sdate ||
                        app.tool.getDatesByIndex(3, app.localTimeToServer)
                            .sdate,
                    endtime:
                        app.tool.parseHash().query.edate ||
                        app.tool.getDatesByIndex(3, app.localTimeToServer).edate
                },
                pageOpt: {
                    sorfield: "",
                    isdesc: false,
    ...
    methods: {
        ...
        _refreshData() {
            this.$refs.commentRankListIScroller.refresh(
                this.pagingOption.params
            );
        },
        // 日期搜索
        changeDate(date) {
        ...
        loadData(ajaxPromise) {
            ajaxPromise.then(res => {
                console.log(res);
                res = {
                    result: { code: 200, msg: "oOpz9" },
                    data: [
                        {
                            itemid: 8918816336148508,
                            itemname: "yidmbd",
                            myrank: 6438766342514620,
                            scope: 8299389493057269
                        }
                    ],
                    page: {
                        pageindex: 1,
        ...
        getCampus(data, _isAll) {
            ...
            if (_isAll) {
                ...
                this.campus.length =
                    data.length > 1 ? "等" + data.length + "个校区" : "";
            }
            // 刷新页面
            // this.pagingOption.params.campusid = _ids.join(',');
            this.pagingOption.params.campusid = _ids;
            this._refreshData();
            this._getCommentRankList();
        },
