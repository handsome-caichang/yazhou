	...
	export default {
		...
		methods: {
	        ...
	        eventMoveHandle(ev) {
	        	...
	        	if (diff > 100) diff = 100
		        if (direction == 3 && this.positionX !== -100 && this.flag) {
		        	this.$refs.content.style.left = diff + 'px'

		        }
		        if (direction == 4 && this.positionX !== 0 && this.flag) {
		        	this.$refs.content.style.left = -(100 - diff) + 'px'
		        }
	        ...
	        eventEndhandle(ev) {
	        	...
	        	if (this.positionX == -100 && this.direction == 3) {
	        		this.moveBack()
	        		return
	        	}
	        	...
	        	this.$refs.content.style.transition = 'all 0.3s'
	        	if (this.diff < -100) this.diff = -100
	        	if (this.diff > 100)  this.diff = 100
	        	if (this.direction == 3 && this.positionX !== -100 && this.flag) {
	        		if (Math.abs(this.diff) <= 50 ) {
	        			this.$refs.content.style.left = 0
	        			this.diff = 0;
	        			
	        		}
	        		if (Math.abs(this.diff) >= 50 && Math.abs(this.diff) < 100) {
	        			this.$refs.content.style.left = -100 + 'px'
	        			this.diff = -100
	        		}
	        	}

	        	if (this.direction == 4 && this.positionX !== 0 && this.flag) {
	        		if (Math.abs(this.diff) <= 50 && this.diff > 0) {
	        			this.$refs.content.style.left = -100 + 'px'
	        			this.diff = 100
	        		}
	        		if (Math.abs(this.diff) >= 50 && Math.abs(this.diff) < 100) {
