                ...
                <div>
                    <div class="card" v-for="(card,index) in list">
                        ...
                        <div class="card-body">
                            ...
                            <div class="voice" v-if="card.Mode=='系统呼入'||card.Mode=='系统呼出'">
                                <div>
                                    通话录音：
                                </div>
                                <voice-recording class="voice-content" :audioFileList="card.recodArr" v-if="parseInt(card.TalkTime)"></voice-recording>
                                <span v-else>00:00秒</span>
                            </div>
                            <div class="body-info">
        ...
        methods: {
            ...
            loadData(ajaxPromise) {
                ajaxPromise.then(res => {
                    ...
                    if (res.errcode == app.errok) {
                        ...
                        this.list.map(obj => {
                            obj.recodArr = [{
                                /*filepath: 'http://voip.800ing.com:8080/voip/record/disk3/201812/25/15574113908-100839806-201812251703-3097770457498070.wav',
                                FileLength: 5*/
                                filepath: obj.RecordFileUrl,
                                FileLength: obj.TalkTime
                            }]
                        })
                    }
                })
            ...
            changeListDate(item, index) {
                ...
                if(app.tool.op("AllowUpdateVisitTime")){
                    ...
                    // item.InviteDate = app.filters.formatDatetime(new Date(),'yyyy-MM-dd hh:mm');
                    let params = {
                        status: 1,
                        id: item.ID,
                        date: app.filters.formatDatetime(new Date(),'yyyy-MM-dd hh:mm')
                    }
                    this.sendData(params, index)
                }
            },
