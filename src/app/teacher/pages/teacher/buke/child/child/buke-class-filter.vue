    ...
    export default {
        ...
        watch: {
            opened(newVal, oldVal) {
                if (newVal) {
                    let para = app.ls.getStorage('bukeSelectedProject');
                    this.scrollerConfig.pagingOption.params.campus = app.ls.getStorage('bukeManageCampusId').campusId;//初始校区id
                    this.scrollerConfig.pagingOption.params.shift = para?para.id:'';//初始校区id
                    this.scrollerConfig.pagingOption.params.shiftName = para?para.name:'';//初始校区id
