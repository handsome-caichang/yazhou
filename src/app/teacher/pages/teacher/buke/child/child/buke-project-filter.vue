    ...
    <action-sheet v-show="opened"
        ...
        <div class="body">
            <div class="data-area">
                ...
                <empty-page class="empty-page" type="2" text="没有课程可选择~" v-show="list.length == 0"></empty-page>
...
<script>
    ...
    import {getBukeFilterProject} from "teacher/api/buke";
        ...
        data() {
            return {
                ...
                scrollerConfig: {
                    ...
                    type: 2,
                    pagingOption: {
                        api: getBukeFilterProject,
                        params: {
                            campus: '',
                            query: '',//课程名称
                            status: 1,//课程状态 1启用 0停用 5启用和停用
                            grade: '',//课程所属年级ID
                            subject: '',//课程所属科目ID
                            category: '',//课程所属类别ID
                            Year: '',//课程所属年份
        ...
        methods: {
            ...
            resetProject() {
                ...
                if (!app.tool.trim(this.queryText) == '') {
                    ...
                    this.isLoading = true;
                    this.$refs.bukeProjectFilterScroller.refresh()
                }
            },
            submit() {
                //再次点击表示取消选择
                ...
                app.ls.setStorage('bukeSelectedProject', result);
                this.$emit('bukeSelectedProject', result);
                this.close();
            }
        },
        beforeDestroy() {
            app.ls.remove('bukeSelectedProject');
        },
        components: {
        ...
        watch: {
            opened(newVal, oldVal) {
                if (newVal) {
                    ...
                    this.$refs.bukeProjectFilterScroller.refresh(this.scrollerConfig.pagingOption.params);
                    this.selectedProject = {};
                } else {//回退清空搜索
                    this.resetProject();
                }
