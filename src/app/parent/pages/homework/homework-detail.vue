...
<template>
	<div class="homework-detail">
		<div v-if="detail&&replyDetail">
                ...
                <div class="body">
                    ...
                    <div class="content">
                        <div class="content-hd task">
                            ...
                            <span class="content-hd-time" 
                                  v-if="allowUpdateJob=='0'">
                                    {{taskWork&&taskWork.createTime}}
                            </span>
                            <span class="edit-btn" 
                                  @click="editHomeWork" 
                                  v-if="allowUpdateJob=='1'&&!comment&&taskWork&&taskWork.isTemp!==1">
                                <svg class="icon" aria-hidden="true">
                        ...
                        <!-- 我的回答：isTemp：1草稿 -->
                        <div class="content-bd pd" v-if="isEdit">
                            <textarea 
                                v-model="content"
                        ...
                        <div class="content-bd pd" v-if="!isEdit">
                            ...
                            <div class="edit-time" v-if="allowUpdateJob=='1'">编辑于{{taskWork.createTime}}</div>
                            <voice-recording
                                class="wrapper"
                                :edit="false"
            ...
            <div class="footer" v-if="computedShowBotm">
                <div class="save-btn" 
                    @click="submit(1)" 
                    v-if="computedSaveMode">存为草稿</div>
	...
	export default {
		...
		data () {
			return {
                ...
                isShowMoreTaskWork: true, //我的回答-展示更多
                taskWorkTypeNums: 0,
                showContent: false, //作业-展示更多
                
                allowUpdateJob: app.sysInfo.AllowUpdateJob,//香港现代教育配置项
                isEdit: true, //是否为编辑状态
			}
		},
		computed: {
            ...
            computedShowBotm() {
                if (this.allowUpdateJob == '1') {
                    if (!this.taskWork || this.taskWork.isTemp == 1) return true
                    if (this.taskWork && this.taskWork.isTemp == 0) {
                        return this.isEdit 
                    } 
                }
                if (this.allowUpdateJob == '0') {
                    if (this.taskWork && this.taskWork.isTemp == 0) {
                        return false
                    } else {
                        return true
            ...
            computedSaveMode() {
                if (this.allowUpdateJob == '0') return true
                if (this.allowUpdateJob == '1') {
                    if (!this.taskWork) {
                        return true
                    } else  {
                        if (this.taskWork.isTemp == 0) {
                            return false //开启配置项且是草稿不能保存
                        } else {
                            return true
                        }
                    }
		...
		methods: {
			...
			getDetail(){ //获取详情
                ...
                Promise.all([getWorkPromise,getReplyPromise]).then(res => {
                    ...
                    if (this.allowUpdateJob === '1' && this.taskWork && this.taskWork.isTemp === 0) {
                        this.isEdit = false
                    }
                    if (this.allowUpdateJob === '0' && this.taskWork && this.taskWork.isTemp === 0) {
                        this.isEdit = false
                    }
                })
            },
            ...
            editHomeWork() {
                ...
                this.$refs.scroll.$el.style.bottom = this.isEdit ? '49px' : 0
                if (!this.isEdit) {
                    this.isLoading = true
                    this.getDetail()
                }
