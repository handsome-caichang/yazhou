...
<template>
	<div class="bargain-container">
		<scroller-base class="bargain" ref="scrollBars" :data="renderData">
			<p class="hotinfo">
				<img class="image" @load="imgLoad" :src="laba">已有{{detail.actinsrecruitcount.visitcount}}人查看，已有{{detail.actinsrecruitcount.enterusercount}}人报名
			</p>
			<div class="body">
				<div class="group-bargain-box" :style="'background-image:url('+ bagBig + ')'">
					<div class="group-bargain">
						...
						<div class="intro-name">{{detail.title}}
                            <div class="share">
                                <svg class="icon " aria-hidden="true" @click="shardClick">
					...
					<div class="content">
						<div class="bargain-start">
							...
							<div class="bargain-start-bottom">
								...
								<div>活动价：
									<span class="price-now">&yen;{{detail.actinsconfigbargain.floorprice}}</span>
								</div>
							</div>
						</div>
					</div>
				...
				<div class="group-item" v-if="isUser && rightBtnText == '已完成'">
					...
					<div class="content">
						<div class="bargain-success">
							<div class="top">您已成功砍掉
								<span class="price-on">{{bargaincompleteinfo.bargainmoney}}</span>元，可享受活动价</div>
							<div class="bottom">
								<span class="price-now">&yen;</span>{{detail.actinsconfigbargain.floorprice}}</div>
						</div>
					</div>
				</div>
				...
				<div class="group-item" v-if="groupmember.length > 0">
                    ...
                    <div class="useritem" v-for="(item,index) in groupmember.length > 2 ? 2 : groupmember.length" :key="index">
                        <div class="left">
                        <img @load="imgLoad" :src="groupmember[index].wechatphoto" alt="">
                        <div class="userinfo">
                        ...
                        <div class="right">砍掉
                        <div class="price">
                            <span style="font-size:12px;">&yen;&nbsp;</span>{{groupmember[index].money}}</div>
                        </div>
                    </div>
                    </div>
                </div>
				...
				<div class="group-item">
					...
					<div class="content">
						<div class="course">
							...
							<img class="courseimg" v-for="(item,index) in activeRuleImgList" @click="previewImg(item,'activeRuleImgList')" :key="index" :src="`${item.FilePath||item.filepath||item.localImgData||item.localId}`">
						</div>
					</div>
				</div>
				...
				<div class="group-item">
					...
					<div class="content">
						<div class="course">
							...
							<img @load="imgLoad" class="courseimg" v-for="(item,index) in campanyInfoImgList" :key="index" @click="previewImg(item,'campanyInfoImgList')" :src="`${item.FilePath||item.filepath||item.localImgData||item.localId}`">
...
<script>
    ...
    import { templateData, templateGroupmember } from "./bargain.js";
    ...
    export default {
        ...
        data() {
            return {
                ...
                bagBig: require("common/img/bargain-bg2.png"),
                titlebag: require("common/img/bargain-titlebg2.png"),
                bargaincompleteinfo: {
                    // 当前砍价的完成情况
            ...
            setTip() {
                this.$nextTick(() => {
                    ...
                    if (
                        ...
                        // console.log(quanbu)
                        // console.log(yiban)
                        // console.log(this.$refs.progressTips.clientWidth)
                        // console.log(proWidth / )
                        let topbaifenbi =
                            Math.floor(
                                proWidth / this.$refs.progressTips.clientWidth * 100
                            ) / 100; //   计算进度条在这个现实金额上面的百分比
                        // if (topbaifenbi)
                        // console.log(topbaifenbi)
                        // this.$refs.progressTips.clientWidth / 2;
                        // console.log(this.$refs.progressBox.clientWidth - proWidth);
                        // console.log(this.$refs.progressTips.clientWidth / 2)
