		...
		<div class="heard"  slot="header">
			...
			<div class="heard-right">
				...
				<div class="right-bottom">
					<div class="price">&#165;{{computedMoney|formatNumber}}</div>
		...
		<div class="course">
			...
			<div class="select-pay" v-if="classInfo.PayType==3">
				...
				<div class="pay" v-for='item in payList' @click="choicePay(item.ID)">
					...
					<div class="select-goods-content-right" :class="{colorAssist:item.ID===selectPayTypeTemp}" v-if="item.ID==2">
						&#165;{{item.Money|formatNumber}}
					</div>
				</div>
...
<script>
	...
	export default{
		...
		computed: {
			...
			computedMoney() {
				return this.selectSpecsTemp.filter(obj => obj.isSelect)[0].Price
			}
		},
		methods: {
	    	...
	    	closeSelect() {
	    		this.$emit('bjSelectBindingsOK', {
	    			...
	    			selectSpecs: this.selectSpecsTemp
	    		})
	    		this.$emit('update:opened', false);
	    	},
	    	cancel() {
			...
			opened(val,oldVal) {
				if (val) {
					...
					if (this.classSelectObj.selectSpecs) {
						this.selectSpecsTemp = app.tool.clone(this.classSelectObj.selectSpecs)
					} else {
						this.selectSpecsTemp = app.tool.clone(this.classInfo.ShiftSpecList)
					}
				}
			},
