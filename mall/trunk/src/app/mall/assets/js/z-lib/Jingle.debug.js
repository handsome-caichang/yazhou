...
var Jingle = J = {
    ...
    settings: {
        ...
        basePageSuffix: '.html',
        dirs: {
            achievements_section: 'master/',
            achievementsDetails_section: 'master/',
            classSaleRate_section: 'master/',
            classSaleRateDetails_section: 'master/',
            classSaleRateSetup_section: 'master/',
            consumeCourse_section: 'master/',
            consumeCourseSchool_section: 'master/',
            master_section: 'master/',
            pwd_section: 'master/',
            receipts_section: 'master/',
            receiptsBranchSchool_section: 'master/',
            receiptsClass_section: 'master/',
            user_section: 'master/',
            table_scroll_section: 'master/',
            cus_addCommunicate_section: 'cus/',
            cus_communicate_section: 'cus/',
            cus_edit_section: 'cus/',
            cus_editCourse_section: 'cus/',
            cus_editDescribe_section: 'cus/',
            cus_editEmployee_section: 'cus/',
            cus_editIntro_section: 'cus/',
            cus_editList_section: 'cus/',
            cus_editSaleMode_section: 'cus/',
            cus_info_section: 'cus/',
            cus_list_section: 'cus/',
            cus_newCusList_section: 'cus/',
            cus_quasiCommList_section: 'cus/',
            cus_update_section: 'cus/',
            cusManage_section: 'cus/',
            cus_analysis_section: 'cus/',
            cus_analysisStatus_section: 'cus/',
            cus_analysisStatusDetail_section: 'cus/',
            cus_saleModeCategory_section: 'cus/',
            cus_analysisSaleMode_section: 'cus/',
            cus_followUpCount_section: 'cus/',
            checkin_section: 'teacher/',
            chooseStudent_section: 'teacher/',
            comment_detail_section: 'teacher/',
            comment_stulist_section: 'teacher/',
            comment_template_section: 'teacher/',
            commonList_section: 'teacher/',
            toTeacher_section: 'teacher/', //ldj
            toTeacher_stulist_section: 'teacher/', //ldj
            courseAttendance_detail_section: 'teacher/',
            courseList_detail_section: 'teacher/',
            courseList_record_section: 'teacher/',
            courseList_section: 'teacher/',
            iframe: 'teacher/',
            main_section: 'teacher/',
            message_detail_section: 'teacher/',
            message_new_section: 'teacher/',
            publicInfo_detail_section: 'teacher/',
            suggestionDetail_section: 'teacher/',
            toTeacher_detail_section: 'teacher/',
            workinfo_section: 'teacher/',
            workinfotj_section: 'teacher/',
            communicateManage_section: 'communicate/',
            communicateManage_search_section: 'communicate/',
            communicateManage_fee_section: 'communicate/',
            communicateManage_add_section: 'communicate/',
            communicateManage_list_section: 'communicate/',
            ranking_section: 'rank/', //ldj
            rankingBrand_section: 'rank/', //ldj
            rankingClass_section: 'rank/', //ldj
            rankingTime_section: 'rank/'
        },
        //page模板的远程路径{#id: href,#id: href}
        remotePage: {},
        // page模板加载完成后，预处理函数(jie扩展)；
...
J.Element = (function($) {
    ...
    var initControlGroup = function() {
            $(document).on('click', 'ul.control-group li', function() {
                var $this = $(this);
    ...
    var _init_badge = function(el) {
        var $el = $(el),
            ...
            orient = $el.data('orient'),
            className = (orient == 'left') ? 'left' : '';
        if ($count.length > 0) {
...
J.Menu = (function($) {
    ...
    var init = function() {
            ...
            /*$asideContainer.find('aside').swipe({
                swipeLeft:function(){
                    if($(this).data('position') != 'right'){
                        hideMenu();
                    }
 ...
 * 侧边菜单的弹出框组件 自己扩展
 */
J.MenuPopup = (function($) {
    function menuPopup(opt) {
        /* cfg示例
            {
                el:'selector',
                title: '标题'，
                ...
                html: html,
                autoScroll: false,
                menuPopupInit: function(){},
                menuPopupShow: function(){},
                colseFn: function(){},
                btn0Fn: function(){},
                btn1Fn: function(){},
                fns: [{
                        selector: '.search_kc > input[type="text"]',
                        evtType: 'input',
                        fn: function () {}
                    },{
                        selector: '.search_con ul > li',
                        evtType: 'click',
                        fn: function () {}
                    }]
            }
        */
        ...
        // this.cfg = opt;
        this.cfg = $.extend({}, this.defaultCfg, opt);
        var conf = this.cfg;


        this.mp = $('<div class="sidePopup"></div>');
        this.mh = $('<div class="sidePopup_head"><div>' + '<div class="colse">' + '<svg class="iconfont_xz" aria-hidden="true"><use xlink:href="#ixz-xiangzuo"></use></svg><span class="sideTitle">筛选</span>' + '</div>' + '</div></div>');
        this.mf = $('<div class="sidePopup_foot flex">' + '<div class="btn0 flex_con">重置</div>' + '<div class="btn1 flex_con">确定</div>' + '</div>');
        this.mb = $('<div class="sidePopup_body"></div>');

        this.iscroll = "";

        this.mh.find('.sideTitle').text(conf.title);

        if (conf.btnTexts) {
            this.btn0 = this.mf.find('.btn0').text(conf.btnTexts[0]);
            if (conf.btnStyles) {
                this.btn0.css(conf.btnStyles[0]);
            }
            this.btn1 = this.mf.find('.btn1').text(conf.btnTexts[1]);
        }

        conf.autoScroll ? $('<div class="scrollWrapper"></div>').appendTo(this.mb).html(conf.html) : this.mb.html(conf.html);

        conf.btnTexts ? this.mp.append(this.mh, this.mf, this.mb) : this.mp.append(this.mh, this.mb);
        this._init(conf);
        this.addEvt(conf);
    };

    menuPopup.prototype.defaultCfg = {
        el: '',
        title: '',
        btnTexts: '',
        html: "",
    ...
    menuPopup.prototype.addEvt = function(conf) {
        var self = this;

        this.mp.find('.colse').click(function(event) {
            self.hide();
        });

        this.mf.find('.btn0').click(function() {
            conf.btn0Fn && conf.btn0Fn.call(self);
            // self.hide();
        })

        this.mf.find('.btn1').click(function() {
            conf.btn1Fn && conf.btn1Fn.call(self);
            self.cfg.btn1AutoClose && self.hide();
        })

        // 绑定用户传入的事件
        $.each(conf.fns, function(index, obj) {
            self.mp.on(obj.evtType, obj.selector, function(event) {
                obj.fn.call(this, self);
            });
        });


        // 响应自定义'sidePopupHideEvent'事件,在此时关闭该弹框
        $(document).on('sidePopupHideEvent', function(event) {
            self.hide();
        });

        self.mb.on('swiperight', function() {
            self.hide();
        });
        ...
        typeof conf.menuPopupInit === 'function' && conf.menuPopupInit.call(this);
    }

    menuPopup.prototype._init = function(conf) {
        this.mp.appendTo('#menuPopup_container')
            .css({ zIndex: zIndex++, "width": conf.width || width });

        conf.autoScroll && (this.iscroll = J.Scroll(this.mp.find('.sidePopup_body')));

        this.el.data('menuPop', this);
    }
    ...
    menuPopup.prototype.hide = function() {
        ...
        if (maskNum > 0) {
            --maskNum === 0 ? $('#menuPopup_mask').hide() : "";
        } else {
            $('#menuPopup_mask').hide();
        }

        // console.log("隐 -> " + maskNum);
    }
    ...
    function init(conf) {
        var $menuPopupMask = $('<div id="menuPopup_mask"></div>').appendTo('body');

        $menuPopupMask.on('click', function() {
            maskNum = 0;
            hide();
        });
    }
    ...
    function show(conf) {
        var mpobj = $(conf.el).data('menuPop') || new menuPopup(conf);

        maskNum += 1;
        // console.log("显 -> " + maskNum); 
        $('#menuPopup_mask').show();
        mpobj.mp.show();
        J.anim(mpobj.mp, {
            translateX: '0%'
        ...
        typeof conf.menuPopupShow === 'function' && conf.menuPopupShow.call(mpobj);

        // console.log(mpobj.iscroll);
        mpobj.iscroll && mpobj.iscroll.scroller.refresh();

        J.hasMenuPopupOpen = true;
    }

    function hide(duration, callback) {
        console.log("sidePopupHideEvent事件触发...");
        $(document).trigger('sidePopupHideEvent');
        J.hasMenuPopupOpen = false;
...
J.Page = (function($) {
    var _formatHash = function(hash) {
            ...
            // jie start
            var id = hash.indexOf('#') == 0 ? hash.substr(1) : hash;
            var dir = J.settings.dirs[id] ? J.settings.dirs[id] + id : id;
            return dir;
            // jie end
        }
        /**
    ...
    var loadSectionTpl = function(hash, callback) {
        ...
        J.settings.showPageLoading && J.jshowMask();
    ...
    var loadContent = function(url, param, callback) {
        return $.ajax({
            ...
            success: function(html) {
                ...
                callback && callback(J.settings.loadHtmlCallback(html));
            },
            // jie start 加载文档片段出错时处理，现在依赖WX JS-SDK;后续花时间改进；
            error: function(xhr, status) {
                    J.settings.showPageLoading && J.hideMask();
                    if (xhr.status === 0 && status !== "abort") { //网络连接失败时处理
                        $(document).trigger('netErrEvent');
                    }
                }
                // jie end
        });
...
J.Router = (function($) {
    ...
    // alert('原始值' + localStorage.getItem("num"));
    // !localStorage.getItem("num") && localStorage.setItem("num",0);
    // history.length === 1 && localStorage.setItem("num",0) 
    // history.length === 1 && alert('我进来了');

    /**
     * 初始化events、state
     */
    var init = function() {
        //使用jq 定义事件event没有state；
    ...
    var _initIndex = function() {
        ...
        console.log("J历史条目数 => " +_history.length);
        console.log("浏览器历史条目数 => " + history.length);
        console.log("当前条目 初始化 => " + parseInt(localStorage.getItem("num")));
        alert("浏览器历史条目数 => " + history.length);
        alert("当前条目 初始化 => " + parseInt(localStorage.getItem("num")));
        console.log("初始化end.................");

        if(history.length > 1 && parseInt(localStorage.getItem("num")) !== 0) {
            var index = parseInt(localStorage.getItem("num"));
        ...
        var indexHash = $section.data('hash') ? $section.data('hash') : '#' + $section.attr('id');

        // _add2History(indexHash,true);
        _add2History(indexHash, true);

        var targetHash = location.hash;
        // jie start 为实现外链跳转后,再回退到我方时，直接展示首页；
        targetHash = "";
        // jie end
    ...
    var _popstateHandler = function(e) {
        ...
        /*if(J.hasPopupOpen){
            J.Popup.close();
            // var newhash=ehash.replace("_section","")+"_detail_section"
            window.history.pushState(J.Util.parseHash(hash),"",hash);
            return;
        };*/

        if ($("#qtmdMask").length > 0) {
            _cancelUploadImage = true;
        };
        // custom end
        ...
        if (e.state && e.state.hash) {
            if (_history[1] && hash === _history[1].hash) { //存在历史记录，证明是后退事件
                ...
                J.hasMenuPopupOpen && J.MenuPopup.hide(); //关闭当前页面的菜单
    ...
    var _targetHandler = function() {
        ...
        var tag = _this.siblings(".alizarin");
        tag.remove();
        // custom end        
        ...
        switch (target) {
            ...
            case 'back':
                // custom start
                if (_history.length != 1 || _history[0].tag == "#main_section") {
                    window.history.go(-1);
                } else {
                    _showSection('#main_section');
                }
                // custom end
    ...
    var _showSection = function(hash) {
            ...
            if (J.hasMenuPopupOpen) { //关闭菜单后再转场
                J.MenuPopup.hide(200, function() {
                    _showSection(hash);
                });
                return;
            }
    ...
    var back = function() {
        ...
        // console.log("J历史条目数 => " +_history.length);
        // console.log("浏览器历史条目数 => " + history.length);
        // alert("当前条目 后退 => " + parseInt(localStorage.getItem("num")));
        // console.log("后退end.................");
    }
    ...
    var _add2History = function(hash, noState) {
        ...
        // if(noState||hashObj.tag == "#main_section"){//不添加浏览器历史记录
        if (noState || hashObj.tag == "#main_section") { //不添加浏览器历史记录
        ...
        // console.log("J历史条目数 => " +_history.length);
        // console.log("浏览器历史条目数 => " + history.length);
        // alert("当前条目 前进 => " + parseInt(localStorage.getItem("num")));
        // console.log("前进end...........................");
    }
...
J.Transition = (function($) {
    ...
    var _finishTransition = function() {
        ...
        $current.find('article.active').trigger('articlehide', [isBack]);
        $target.find('article.active').trigger('articleshow', [isBack]);
...
(function($) {
    ...
    J.jshowMask = function(text) {
            J.Popup.jloading(text);
        }
        /**
         * 关闭loading框
         */
    J.hideMask = J.jhideMask = function() {
            J.Popup.close(true);
        }
        /**
         *  显示消息
    ...
    J.alert = function(title, content, btnName) {
        J.Popup.alert(title, content, btnName);
    }
    J.confirm = function(title, content, btn1Fun, btn0Fun, btn1Name, btn0Name, isPadding) {
            J.Popup.confirm(title, content, btn1Fun, btn0Fun, btn1Name, btn0Name, isPadding);
        }
        /**
         * 弹出窗口
         * @param options
...
J.Popup = (function($) {
    var _popup, _mask, transition, clickMask2close,
        ...
        TEMPLATE = {
            ...
            //          alert : '<div class="popup-title">{title}</div><div class="popup-content">{content}</div><div id="popup_btn_container"><a data-target="closePopup">{ok}</a></div>',
            ...
            alert: '<div class="popup-title">{title}</div><div class="popup-content prbl">{content}</div><div id="popup_btn_container"><a data-target="closePopup">{ok}</a></div>',
            confirm: '<div class="popup-title">{title}</div><div class="popup-content prbl">{content}</div><div id="popup_btn_container"><a class="cancel">{cancel}</a><a class="confirm">{ok}</a></div>',
            loading: '<i class="icon spinner"></i><p>{title}</p>'
                // loading : '<div style="width: 150px;position: absolute;top: 50%;left: 50%;margin:-100px -75px;text-align:center"><img src="img/loading.jpg" width="150px"><p>{title}</p></div>'
        };
    ...
    var show = function(options) {
        ...
        if (settings.maxHeight) {
            _popup.css('max-height', settings.maxHeight);
        ...
        //显示获取容器高度，调整至垂直居中
        if (settings.pos == 'center' || settings.pos == 'center-second') {
     ...
     * @param title 标题
     * @param content 内容
     * @param btn1Fun 确定按钮handler
     * @param btn0Fun 取消按钮handler
     * @param btn1Name 确定按钮文字
     * @param btn0Name 取消按钮文字
     * @param isPadding content部分是否需要padding
     */
    var confirm = function(title, content, btn1Fun, btn0Fun, btn1Name, btn0Name, noPadding) {
        var markup = TEMPLATE.confirm.replace('{title}', title).replace('{content}', content).replace('{ok}', btn1Name || '确定').replace('{cancel}', btn0Name || '取消');
        if (noPadding) {
            markup = markup.replace('prbl', '');
        }
        show({
            html: markup,
            pos: 'center',
        ...
        //          hide();
        //          btn0Fun&&btn0Fun.call(this);
        //      });
        $('#popup_btn_container [class="confirm"]').click(function() {
            hide();
            btn1Fun && btn1Fun.call(this);
        });
        $('#popup_btn_container [class="cancel"]').click(function() {
            hide();
            btn0Fun && btn0Fun.call(this);
        });
    }
    ...
    var jloading = function() {
        var markup = '<img src="img/loading3.gif" height="60px" style="position: absolute;top: 50%;left: 50%;margin-left: -30px;margin-top: -30px;">'
            // var markup = TEMPLATE.loading.replace('{title}',text||'加载中...');
        show({
            html: markup,
            pos: 'jloading',
    ...
    var calendar = function(selector, options) {
            ...
            var _renderBody = function(date) {
                var firstDay = _fisrtDayOfMonth(date),
                    ...
                    rows = Math.ceil((days - (7 - firstDay + 1)) / 7) + 1,
                    beginDate,
                    html = '';
                currentYear = date.getFullYear();
                currentMonth = date.getMonth();
                beginDate = new Date(currentYear, currentMonth, 2 - firstDay); //日历开始的日期
                html += '<table><tbody>';
            ...
            var _renderDay = function(date, month) {
                ...
                return otherMonth ? '<td>&nbsp;</td>' : '<td  class="' + classname + '" data-date= ' + dateStr + '><span data-selected="selected">' + dayStr + '</span></td>';
            }
...
(function($) {
    function slider(selector, showDots) {
        ...
        var _init = function() {
            ...
            // window.Zepto && (slideWidth = wrapper.offset().width);
            window.jQuery && (slideWidth = wrapper.width());
...
(function($) {
    ...
    function Refresh(selector, type, callback) {
        ...
        var _init = function(opts) {
            ...
            var refreshTpl = '<div class="refresh-container"><span class="refresh-icon icon ' + opts.onPullIcon + '"></span><span class="refresh-label">' + opts.pullText + '</span>' + (opts.refreshTip ? '<div class="refresh-tip">' + opts.refreshTip + '</div>' : '') + '</div>';
            if (isPullDown) {
...
(function($) {
    ...
    function upDownRefresh(selector, type, callback) {
        var iscroll, scroller, refreshEl, iconEl, labelEl, topOffset, bottomOffset, isPullDown, down_refreshEl, up_refreshEl,
            options = {
                selector: undefined,
                type: 'pullDown', //pullDown|pullUp 默认为pullDown
                minPullHeight: 10, //拉动的像素相对值，超过才会翻转
                ...
                down_pullText: "下拉刷新...",
                down_releaseText: "松开立即刷新...",
                down_refreshText: "刷新中...",
                down_onPullIcon: 'arrow-down-2',
                down_callback: undefined,

                up_pullText: "上拉加载更多...",
                up_releaseText: "松开开立即加载...",
                up_refreshText: "加载中...",
                up_onPullIcon: 'arrow-up-3',
                up_callback: undefined,
            },
            status = "refresh"; //刷新refresh，加载更多，loadmore，还有hasmore 最后一页nomore
        //装载配置
        if (typeof selector === 'object') {
            $.extend(options, selector);
        } else {
        ...
        var _init = function(opts) {
            ...
            var down_refreshTpl = '<div class="refresh-container"><span class="refresh-icon icon ' + opts.down_onPullIcon + '"></span><span class="refresh-label">' + opts.down_pullText + '</span>' + (opts.refreshTip ? '<div class="refresh-tip">' + opts.refreshTip + '</div>' : '') + '</div>';

            var up_refreshTpl = '<div class="refresh-container" style="display:none;"><span class="refresh-icon icon ' + opts.up_onPullIcon + '"></span><span class="refresh-label">' + opts.up_pullText + '</span></div>';

            down_refreshEl = $(down_refreshTpl).prependTo(scroller);
            // topOffset = down_refreshEl.height(); //高度只有一半，不晓得是哦该
            down_refreshEl.length && (topOffset = down_refreshEl[0].clientHeight);
            up_refreshEl = $(up_refreshTpl).appendTo(scroller);
            up_refreshEl.length && (bottomOffset = up_refreshEl[0].clientHeight);

            iconEl = down_refreshEl.find('.refresh-icon');
            labelEl = down_refreshEl.find('.refresh-label');
        }
            ...
            return J.Scroll(opts.selector, {
                ...
                onScrollMove: function() {
                    ...
                    if (this.distY > 0) {
                        ...
                        opts.pullText = opts.down_pullText;
                        opts.refreshText = opts.down_refreshText;
                        opts.releaseText = opts.down_releaseText;
                        opts.onPullIcon = opts.down_onPullIcon;
                        opts.callback = opts.down_callback;
                        isPullDown = true;
                    } else {
                        if (status != "hasmore") {
                            return;
                        ...
                        // console.log(this.distY);
                        iconEl = up_refreshEl.find('.refresh-icon');
                        labelEl = up_refreshEl.find('.refresh-label')
                        opts.pullText = opts.up_pullText;
                        opts.refreshText = opts.up_refreshText;
                        opts.releaseText = opts.up_releaseText;
                        opts.onPullIcon = opts.up_onPullIcon;
                        opts.callback = opts.up_callback;
                        isPullDown = false;
                    }

                    // console.log(iconEl.siblings('span').text())

                    if (this.y > opts.minDownHeight && isPullDown && !iconEl.hasClass(opts.onReleaseIcon)) {
                        iconEl.addClass(opts.onReleaseIcon);
                        labelEl.html(opts.releaseText);
                        this.minScrollY = 0;
                    } else if (this.y < opts.minDownHeight && isPullDown && iconEl.hasClass(opts.onReleaseIcon)) {
                        iconEl.removeClass(opts.onReleaseIcon);
                        labelEl.html(opts.pullText);
                        this.minScrollY = -topOffset;
                    } else if (this.y < (this.maxScrollY - opts.minUpHeight) && !isPullDown && !iconEl.hasClass(opts.onReleaseIcon)) {
                        iconEl.addClass(opts.onReleaseIcon);
                        labelEl.html(opts.releaseText);
                        this.maxScrollY = this.maxScrollY;
                        // } else if (this.y > (this.maxScrollY + opts.minUpHeight) && !isPullDown && iconEl.hasClass(opts.onReleaseIcon)) {
                        ...
                        labelEl.html(opts.pullText);
                        // this.maxScrollY = 0;
                        // this.maxScrollY = this.maxScrollY + topOffset;
                    }
                ...
                onScrollEnd: function() {
                    ...
                    if (iconEl.hasClass(opts.onReleaseIcon)) {
                        ...
                        setTimeout(function() { //解决在chrome下onRefresh的时候文本无法更改的问题。奇怪的问题！
                            // opts.callback.call(_this);
                            isPullDown ? _this.fireEvent("pageRefreshEvent") : ((status == "hasmore") && _this.fireEvent("scrollBottomEvent"));
                        }, 1);
                    }
                },
                onRefresh: function() {
        ...
        /* 增加自定义事件能力 */
        $.extend(iscroll.scroller, new Event());
        // console.log(iscroll.scroller);

        //添加加载到页尾的函数 0118 cxf
        iscroll.changeState = function(s) {
            status = s;
            var hideMore = (s == "nomore") || (s == "refresh");
            up_refreshEl[hideMore ? "hide" : "show"]();
        }
        return iscroll;
    }
     ...
     * @param callback 回调函数
     */
    J.upDownRefresh = function(selector, type, callback) {
        var el, jRefreshId;
        if (selector.selector) {
            el = $(selector.selector)
        } else {
        ...
        jRefreshId = el.data('_jieRefresh_');
        //因上拉下拉可能会使用的比较频繁，故缓存起来节省开销,亦可防止重复绑定
        if (jRefreshId && refreshCache[jRefreshId]) {
            return refreshCache[jRefreshId];
        } else {
            jRefreshId = '_jieRefresh_' + index++;
            el.data('_jieRefresh_', jRefreshId);
            var refresh = new upDownRefresh(selector, type, callback);
            return refreshCache[jRefreshId] = refresh;
            //  {
            //     scroller : refresh.scroller,
            //     destroy : function(){
            //         delete refreshCache[jRefreshId];
